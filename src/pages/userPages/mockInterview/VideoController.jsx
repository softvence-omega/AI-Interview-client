...
//       console.log("calling api for video analysis ..............+++++++++++===========>>>>>>")
  ...
  (
    {
      ...
      handleNextQuestion,
      setIsProcessing,
      onStopRecording, // Added prop for consistent recording stop
    },
    ref
  ) => {
    const { user } = useAuth();
    ...
    // Countdown before recording starts
    useEffect(() => {
      if (!isVideoState) return; // Skip countdown if video state is off
      if (countdown > 0) {
        const timer = setInterval(() => {
          setCountdown((prev) => prev - 1);
        }, 1000);
    ...
    // Recording timer
    useEffect(() => {
      if (!isVideoState || !isRecording) return; // Skip timer if video state is off or not recording
      if (recordingTimeLeft > 0) {
        ...
        stopRecording(); // Trigger stopRecording when timer expires
      }
    ...
    const startRecording = async () => {
      if (!isVideoState) return; // Prevent recording if video state is off
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: true,
          audio: true,
    ...
    const callAIApiforVideoAnalysis = async (videoBlob) => {
      ...
      console.log("<<<=======calling api for annalisids ======>>>>>")
      const API_URL = `${aiBaseUrl}/video_process/process-video/`;
      const activeQuestion = aboutMeTry ? aboutMeQuestion : question;
    ...
    // Render nothing when isVideoState is false, as ContentSection handles AI response
    if (!isVideoState) {
      return null;
    }
